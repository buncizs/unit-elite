...
const dir = path.join(root, "integrations", "9router");
const cmdPath = path.join(dir, "stop-9router.cmd");
const ps1Path = path.join(dir, "stop-9router.ps1");

if (!fs.existsSync(cmdPath)) {
  console.error("ERROR: stop-9router.cmd not found: " + cmdPath);
  process.exit(2);
}

const suffix = ".pre-r5c-ownership-fallback.bak";
for (const f of [cmdPath, ps1Path]) {
  if (fs.existsSync(f)) {
    const bak = f + suffix;
...
$tk = Start-Process -FilePath "$env:SystemRoot\System32\taskkill.exe" -ArgumentList @('/PID', [string]$targetPid, '/T', '/F') -Wait -PassThru -WindowStyle Hidden
if ($tk.ExitCode -ne 0) {
...
// On Windows, validate the generated PowerShell with the native parser BEFORE
// any production file is replaced. If parsing fails, abort with no modification.
if (process.platform === "win32") {
  const tmpPs1 = path.join(os.tmpdir(), `unit-elite-stop-9router-${process.pid}.ps1`);
  fs.writeFileSync(tmpPs1, ps1.replace(/\r?\n/g, "\r\n"), "utf8");
  try {
    const parserCmd = [
      "$tokens=$null",
      "$errors=$null",
      ...
      "  exit 1",
      "}",
      "Write-Output 'POWERSHELL_SYNTAX_PASS'"
    ].join("; ");

    const out = execFileSync(
      "powershell.exe",
      ["-NoProfile", "-Command", parserCmd, tmpPs1],
      { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }
    );
    process.stdout.write(out);
  } catch (e) {
    console.error("ERROR: generated stop-9router.ps1 failed native PowerShell syntax validation.");
  ...
  console.log("POWERSHELL_SYNTAX_CHECK_SKIPPED_NON_WINDOWS");
}

fs.writeFileSync(ps1Path, ps1.replace(/\r?\n/g, "\r\n"), "utf8");
fs.writeFileSync(cmdPath, cmd, { encoding: "ascii" });

console.log("PATCHED=" + ps1Path);
console.log("PATCHED=" + cmdPath);
console.log("PATCH_OK");
console.log("STOP_POLICY=verified cli.js ancestor OR verified canonical custom-server.js listener fallback");
console.log("FORBIDDEN=taskkill by image name; unverified PID kill");
