...
// Validate generated PowerShell BEFORE any backup/write to production files.
if (process.platform === "win32") {
  const tmpPs1 = path.join(os.tmpdir(), `unit-elite-stop-9router-${process.pid}.ps1`);
  const tmpValidator = path.join(os.tmpdir(), `unit-elite-ps-validator-${process.pid}.ps1`);
  ...
  fs.writeFileSync(tmpValidator, validator, "utf8");
  ...
  try {
    ...
    try { fs.unlinkSync(tmpValidator); } catch {}
    process.exit(6);
  }
...
// Only after validation passes do we create backups and replace production files.
const suffix = ".pre-r5c-ownership-fallback-v4.bak";
